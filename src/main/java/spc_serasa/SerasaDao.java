...
public class SerasaDao extends Dao<ClienteSerasa> {
	...
	public List<ClienteSerasa> listar() {
		...
		try (PreparedStatement pstmp = conn.prepareStatement(sql)) {
			...
			while (resultSet.next()) {
				...
				ClienteSerasa clienteSerasa = new ClienteSerasa(status, id, nomeDevedor, tipoPessoa, documento,
						natureza, valor, dataCadastro, horaCadastro, dataVencimento, operacao);
	...
	public void inserir(List<ClienteSerasa> clientes) {
		...
		String sql = "insert into serasa(serasa_status, serasa_codigo, serasa_nome, serasa_tipo,"
				...
				+ "serasa_data_vencimento, serasa_operacao)" + " values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	...
	public void atualizaNegativado(List<ClienteSerasa> clientes) {
		...
		String sql1 = "select inst.contrno as contrato from inst inner join custp on(inst.custno = custp.no) where custp.cpf_cgc = ?";
		String sql2 = "update itxa set itxa.status = case "
				...
				+ "else 0 " + "end   where itxa.contrno = ? and itxa.status not in(?, ?)";
		...
		try {
			...
			try (PreparedStatement psm1 = conn.prepareStatement(sql1);
				...
				for (ClienteSerasa cliente : clientes) {
					...
					psm1.setString(1, cliente.getCpf());

					try (ResultSet resultSet = psm1.executeQuery()) {

						while (resultSet.next()) {

							long contrato = resultSet.getLong(1);
			...
			throw new RuntimeException("Erro ao atualizar clientes negativados do Serasa", ex);
		}
